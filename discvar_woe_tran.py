...
class charWoe(object):
    ...
    def char_woe(self):
        # 得到每一类样本的个数，且加入平滑项使bad和good都不为0
        ...
        for col in self.vars:
            ...
            for (k, v) in data.items():
                ...
                # 字典中嵌套字典,返回{-9997:{0: 31877, 1: 2128}}
                dic[value][int(dp)] = v
    ...
    def combine_box_char(self, dic):
        ...
        while len(dic) >= 10:
            # k是特征值，v["bad_rate"]是与特征值对应的负样本占比
            bad_rate_dic = {k: v["bad_rate"]
                     for (k, v) in dic.items()}
            # 按照负样本占比排序，因为离散型变量是无序的
            # 可以直接写成负样本占比递增的形式
        ...
        while min_cnt < self.nrows * 0.05 and len(dic) > 5:
            ...
            dic["%s|%s" % (k1, k2)]["1"] = dic[k1].get("1", 0) + dic[k2].get("1", 0)
            dic["%s|%s" % (k1, k2)]["cnt"] = dic[k1]["cnt"] + dic[k2]["cnt"]
            dic["%s|%s" % (k1, k2)]["bad_rate"] = round(dic["%s|%s" % (k1, k2)]["1"] /
                     dic["%s|%s" % (k1, k2)]["cnt"],5)
            # 删除旧的key
            del dic[k1], dic[k2]
            # 当前最小的箱的样本个数
...
if __name__=='__main__':
    '''使用方法
    '''
    chawoe=charWoe({'dev':d3},'target',1,['B1'])
    x1=chawoe.char_woe()
    x1['dev'].B1_woe.value_counts()